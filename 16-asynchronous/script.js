...
2. Do 'reverse geocoding' of the provided coordinates. Reverse geocoding means to convert coordinates to
    a meaningful location, like a city and country name. Use this API to do reverse geocoding: https://geocode.xyz/api.
...
3. Once you have the data, take a look at it in the console to see all the attributes that you recieved 
    about the provided location. Then, using this data, log a messsage like this to the console: 'You are in 
    Berlin, Germany'
...
Promise.resolve('Promise Resolve').then(x => console.log(x));
...
const whereAmI = function () {
  getPosition()
    ...
    .then(data => {
      ...
      return fetch(`https://restcountries.com/v3.1/name/${data.prov}`);
    })
    .then(res => {
      if (!res.ok) throw new Error(`Country not found (${res.status})`);
      console.log(res);
...
TEST DATA: Images in the img folder. Test the error handler by passing a wrong image path. Set the network 
speed to 'Fast 3G' in the dev tools Network tab, otherwise images load too fast.
